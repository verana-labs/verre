...
import { Constructor, MultiBaseEncoder, parseWithErrorHandling, VarintEncoder } from '../../../../utils'
...
export class PublicJwk<Jwk extends SupportedPublicJwk = SupportedPublicJwk> {
  ...
  public static fromUnknown(jwkJson: unknown) {
    ...
    const publicJwk = publicJwkFromPrivateJwk(
      parseWithErrorHandling(zKmsJwkPublic, jwkJson, 'jwk is not a valid jwk'),
    )
    ...
    if (publicJwk.kty === 'RSA') {
      ...
      throw new Error(`Unsupported kty '${publicJwk.kty}' for creating jwk instance`)
    }
  ...
  public static fromPublicJwk<Jwk extends KmsJwkPublicAsymmetric>(jwk: Jwk) {
    return PublicJwk.fromUnknown(jwk) as PublicJwk<
      ExtractByJwk<SupportedPublicJwk, Jwk> extends never
        ? SupportedPublicJwk
        : ExtractByJwk<SupportedPublicJwk, Jwk>
    >
  }
  ...
  public get keyId(): string {
    ...
    throw new Error('Unable to determine keyId for jwk')
  }
  ...
  public get signatureAlgorithm() {
    ...
    if (!alg) {
      throw new Error(`${getJwkHumanDescription(this.jwk.jwk)} has no supported signature algorithms`)
    }
  ...
  public static fromFingerprint(fingerprint: string) {
    ...
    if (!PublicJwkClass) {
      throw new Error(`Unsupported multicodec public key with prefix '${code}'`)
    }
  ...
  public convertTo(
    ...
    if (!this.is(Ed25519PublicJwk) || type !== X25519PublicJwk) {
      throw new Error('Unsupported key conversion. Only Ed25519 to X25519 is supported.')
    }
  ...
  public static supportedPublicJwkClassForSignatureAlgorithm(
    alg: KnownJwaSignatureAlgorithm,
  ): SupportedPublicJwkClass {
    ...
    if (!supportedPublicJwkClass) {
      throw new Error(`Could not determine supported public jwk class for alg '${alg}'`)
    }
