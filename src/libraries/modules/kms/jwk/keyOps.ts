...
export const zKnownJwkUse = z.union([
  z.literal('sig').describe('signature'),
  z.literal('enc').describe('encryption'),
])
...
export function assertKeyAllowsDerive(jwk: KmsJwkPrivate | KmsJwkPublic) {
  if (!keyAllowsDerive(jwk)) {
    throw new Error(`${getJwkHumanDescription(jwk)} usage does not allow key derivation operations`)
  }
}
...
export function assertKeyAllowsVerify(jwk: KmsJwkPrivate | KmsJwkPublic) {
  if (!keyAllowsVerify(jwk)) {
    throw new Error(`${getJwkHumanDescription(jwk)} usage does not allow verification operations`)
  }
}
...
export function assertKeyAllowsSign(jwk: KmsJwkPrivate | KmsJwkPublic) {
  if (!keyAllowsSign(jwk)) {
    throw new Error(`${getJwkHumanDescription(jwk)} usage does not allow signing operations`)
  }
}
...
export function assertKeyAllowsEncrypt(jwk: KmsJwkPrivate | KmsJwkPublic) {
  if (!keyAllowsEncrypt(jwk)) {
    throw new Error(`${getJwkHumanDescription(jwk)} usage does not allow encryption operations`)
  }
}
...
export function assertKeyAllowsDecrypt(jwk: KmsJwkPrivate | KmsJwkPublic) {
  if (!keyAllowsDecrypt(jwk)) {
    throw new Error(`${getJwkHumanDescription(jwk)} usage does not allow decryption operations`)
  }
}
