...
export function assertJwkAsymmetric(
  ...
  if (!isJwkAsymmetric(jwk)) {
    if (keyId) {
      throw new Error(`Expected jwk with keyId ${keyId} to be an assymetric jwk, but found kty 'oct'`)
    }
    throw new Error("Expected jwk to be an assymetric jwk, but found kty 'oct'")
  }
}
