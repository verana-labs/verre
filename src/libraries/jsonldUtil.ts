...
import { JsonObject, JsonValue, SECURITY_CONTEXT_URL, W3cJsonLdVerifiableCredential } from '@credo-ts/core'
...
export function assertOnlyW3cJsonLdVerifiableCredentials(
  ...
  if (credentials.some(c => !(c instanceof W3cJsonLdVerifiableCredential))) {
    throw new Error('JSON-LD VPs can only contain JSON-LD VCs')
  }
}
