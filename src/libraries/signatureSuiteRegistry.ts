...
export class SignatureSuiteRegistry {
  ...
  public constructor(suites: Array<SuiteInfo | 'default'>) {
  ...
  public getByProofType(proofType: string) {
    ...
    if (!suiteInfo) {
      throw new Error(`No signature suite for proof type: ${proofType}`)
    }
  ...
  public getVerificationMethodTypesByProofType(proofType: string): string[] {
    ...
    if (!suiteInfo) {
      throw new Error(`No verification method type found for proof type: ${proofType}`)
    }
